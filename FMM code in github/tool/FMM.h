...
public:
  ...
  int Min_fingerprint_length;
  int real_fingerprint_length;
  int64_t mod_Min_fingerprint_length,mod_real_fingerprint_length;
  HashFamily *hasher;
  size_t hashIndex{0};

  double get_bit_per_item(){
    return (arrayLength*real_fingerprint_length)/(size*1.0);
  }
  
  double get_all_bits_num(){
    return (arrayLength*real_fingerprint_length);
  }
  ...
  explicit XorBinaryFuseFilter(const size_t size,const int Min_fingerprint_length, const int real_fingerprint_length) {
    hasher = new HashFamily();
    this->Min_fingerprint_length=Min_fingerprint_length;
    this->real_fingerprint_length=real_fingerprint_length;
    this->mod_Min_fingerprint_length=1<<Min_fingerprint_length;
    this->mod_real_fingerprint_length=1<<real_fingerprint_length;
    this->size = size;
  ...
  FingerprintType show_store_data(const ItemType &key){
    ...
    if(temp>>Min_fingerprint_length == ((hash%mod_real_fingerprint_length)>>Min_fingerprint_length)){
      return (temp % mod_Min_fingerprint_length);
    }
    else
      return -1;  
  }
...
Status XorBinaryFuseFilter<ItemType, FingerprintType, HashFamily>::AddAll(const ItemType *keys,const ItemType *category,const size_t start, const size_t end) 
{
  ...
  for (int i = reverseOrderPos - 1; i >= 0; i--) {
    ...
    FingerprintType xor2 = (((hash%mod_real_fingerprint_length)>>Min_fingerprint_length)<<Min_fingerprint_length)+record[hash]%mod_Min_fingerprint_length;//--------------------------> 8
    h012[0] = getHashFromHash(hash, 0);
    h012[1] = getHashFromHash(hash, 1);
    h012[2] = getHashFromHash(hash, 2);
    h012[3] = h012[0];
